...
with open('FSAS20190917/4/lci.ins','rt') as f:
	for line in f:
		if line[0] == "%":
			continue
		value=line.split()
		t.append(float(value[0+1]))
		X.append(float(value[1+1]))
		Y.append(float(value[2+1]))
		Z.append(float(value[3+1]))
...
plt.scatter(L,B,color='green',s=30)
plt.title('Trajection',color='black',fontsize=40)
plt.grid(ls='-')
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.xlabel('Longitude/(rad)',fontsize=35)
plt.ylabel('Latitude/(rad)',fontsize=35)
